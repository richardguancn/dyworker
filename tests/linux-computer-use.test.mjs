...
linuxTest("Linux 无障碍树只读取已绑定的具体窗口", async () => {
  ...
  assert.match(helper, /describe\(\s*window_root\s*\)/);
  // python 源码可能因行宽折行（element_at(\n  window_root, …），断言必须容忍空白
  assert.match(helper, /element_at\(\s*window_root,/);
  assert.doesNotMatch(helper, /describe\(\s*application\s*\)/);
...
linuxTest("Linux 安装工具获得系统授权后执行固定安装并重新检查", async (t) => {
  ...
  const scripts = {
    ...
    pkexec: [
      ...
      "while [ \"$1\" != \"--locked-dependency-install-worker\" ] && [ \"$#\" -gt 0 ]; do",
      "  case \"$1\" in",
